"""Build script for Lab 02: Data Ingestion — How the Poller Works.

Generates ``02_data_ingestion.ipynb`` using nbformat. Run this script to
...
OUTPUT_PATH = "labs/02_data_ingestion.ipynb"


def build() -> nbf.NotebookNode:
    """Construct the Lab 02 notebook programmatically."""
    ...
    # ── Cell 3: Section 1 — Poller Architecture ────────────────────────
    cells.append(
        nbf.v4.new_markdown_cell(
            "---\n"
            "\n"
    ...
    # ── Cell 5: Section 2 — Configuration ──────────────────────────────
    cells.append(
        nbf.v4.new_markdown_cell(
            "---\n"
            "\n"
            ...
            "\n"
            "The `PollerConfig` aggregates all sub-configs. Let's inspect it to "
            "understand what's configurable."
        )
    )

    # ── Cell 6: Inspect PollerConfig ───────────────────────────────────
    cells.append(
        nbf.v4.new_code_cell(
    ...
    # ── Cell 9: Section 3 — Raw odds data ──────────────────────────────
    cells.append(
        nbf.v4.new_markdown_cell(
            "---\n"
            "\n"
            ...
            "\n"
            "The `odds_ticks` hypertable is where all odds data lands. Each row "
            "represents a single price point for a selection at a bookmaker.\n"
            "\n"
            "Columns: `sport`, `league`, `event_id`, `book`, `market`, "
            "`selection`, `price`, `line`, `ts`, `source_raw`."
        )
    )

    # ── Cell 10: Query latest odds ──────────────────────────────────────
    cells.append(
        nbf.v4.new_code_cell(
    ...
    # ── Cell 11: odds_ticks column details ──────────────────────────────
    cells.append(
        nbf.v4.new_code_cell(
    ...
    # ── Cell 12: Section 4 — Aggregate by book/sport ──────────────────
    cells.append(
        nbf.v4.new_markdown_cell(
            "---\n"
            "\n"
            ...
            "\n"
            "Understanding which books and sports have the most data helps you "
            "identify coverage gaps and plan analyses."
        )
    )

    # ── Cell 13: Aggregate query ────────────────────────────────────────
    cells.append(
        nbf.v4.new_code_cell(
    ...
    # ── Cell 15: Section 5 — Raw injury data ──────────────────────────
    cells.append(
        nbf.v4.new_markdown_cell(
            "---\n"
            "\n"
            ...
            "\n"
            "The `injuries` hypertable stores ESPN injury reports. Each row "
            "represents a single player injury status at a point in time.\n"
            "\n"
            "Columns: `sport`, `player_id`, `player_name`, `team`, `position`, "
            "`status`, `detail`, `ts`, `source_raw`."
        )
    )

    # ── Cell 16: Query latest injuries ──────────────────────────────────
    cells.append(
        nbf.v4.new_code_cell(
    ...
    # ── Cell 17: Section 6 — Aggregate injuries by team ────────────────
    cells.append(
        nbf.v4.new_markdown_cell(
            "---\n"
            "\n"
            ...
            "\n"
            "Understanding injury patterns by team helps identify which teams "
            "have the most health concerns."
        )
    )

    # ── Cell 18: Injury aggregation ──────────────────────────────────────
    cells.append(
        nbf.v4.new_code_cell(
    ...
    # ── Cell 20: Section 7 — Hypertable internals ──────────────────────
    cells.append(
        nbf.v4.new_markdown_cell(
            "---\n"
            "\n"
    ...
    # ── Cell 23: Section 8 — Compression and retention ──────────────────
    cells.append(
        nbf.v4.new_markdown_cell(
            "---\n"
            "\n"
    ...
    # ── Cell 26: Section 9 — Manual poll trigger ───────────────────────
    cells.append(
        nbf.v4.new_markdown_cell(
            "---\n"
            "\n"
            ...
            "\n"
            "The `run_poll_cycle()` function runs all enabled pollers once. "
            "This is useful for:\n"
            "\n"
            ...
            "\n"
            "**Note**: If the Odds API key is not set, the odds poller will be "
            "skipped. The ESPN source requires no API key."
        )
    )

    # ── Cell 27: Count before poll ──────────────────────────────────────
    cells.append(
        nbf.v4.new_code_cell(
    ...
    # ── Cell 30: Section 10 — Poller health and runs ───────────────────
    cells.append(
        nbf.v4.new_markdown_cell(
            "---\n"
            "\n"
    ...
    # ── Cell 34: Section 11 — Poller logs ──────────────────────────────
    cells.append(
        nbf.v4.new_markdown_cell(
            "---\n"
            "\n"
            ...
            "\n"
            "Every poll cycle writes structured log entries to `poller_logs`. "
            "Use `get_poller_logs()` to tail recent log lines for a specific poller."
        )
    )

    # ── Cell 35: Get poller logs ────────────────────────────────────────
    cells.append(
        nbf.v4.new_code_cell(
    ...
    cells.append(
        nbf.v4.new_markdown_cell(
            ...
            "\n"
            "1. **Find the longest-running poller** — Query `poller_runs` to find "
            "which poller has the highest average `finished_at - started_at` duration:\n"
            "   ```sql\n"
            "   SELECT poller_name,\n"
            ...
            "   ```\n"
            "\n"
            "2. **Which book has the most NFL data?** — Find the bookmaker with the "
            "most `odds_ticks` rows for `sport = 'americanfootball_nfl'`.\n"
            "\n"
            "3. **Injury report timeline** — For a specific team, show the timeline "
            "of injury status changes:\n"
            "   ```sql\n"
    ...
    cells.append(
        nbf.v4.new_markdown_cell(
            ...
            "### Next Steps\n"
            "\n"
            "Continue to **Lab 03: Expected Value Calculation** to learn how to "
            "convert raw odds into implied probabilities and find +EV bets.\n"
            "\n"
            "---\n"
            "\n"
