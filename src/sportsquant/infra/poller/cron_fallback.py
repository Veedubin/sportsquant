"""Cron-style fallback scheduler for SportsQuant poller.
...
setting ``SPORTSQUANT_POLLER_SCHEDULER=cron``.
...
from sportsquant.infra.poller.tasks import TaskResult, run_poll_cycle
...
def format_cycle_summary(results: list[TaskResult]) -> str:
    ...
    Args:
        results: The list of :class:`TaskResult` objects from
            :func:`~sportsquant.infra.poller.tasks.run_poll_cycle`.
