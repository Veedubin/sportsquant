"""Poller task orchestration for SportsQuant v0.2.0.
...
The :mod:`~sportsquant.infra.poller.flows` module (Wave 2) will wrap them as
Prefect tasks/flows; :mod:`~sportsquant.infra.poller.cron_fallback` will call
them in a loop.
"""
...
from sportsquant.infra.db.connection import DatabasePool, get_pool
from sportsquant.infra.db.writers import (
...
from sportsquant.infra.poller.sources.espn_injuries import (
    ESPNInjuriesError,
    fetch_and_parse as espn_fetch_and_parse,
)
from sportsquant.infra.poller.sources.odds_api import (
    OddsAPIError,
...
async def run_odds_api_poll(config: PollerConfig, sport: str) -> TaskResult:
    ...
    Args:
        config: Fully populated :class:`~sportsquant.infra.poller.config.PollerConfig`.
