"""Async batch writers for SportsQuant TimescaleDB tables.

Provides high-throughput, idempotent write helpers for the poller pipeline.
Bulk inserts use asyncpg's ``COPY`` protocol via
:meth:`~sportsquant.infra.db.connection.DatabasePool.copy_records_to_table`
...
:class:`~sportsquant.infra.db.connection.DatabasePool` as its first argument.
...
def _coerce_timestamp(value: Any) -> datetime | str:
    ...
    :func:`~sportsquant.util.time_utils.utc_now_iso`), so we parse those here.
