"""Asyncpg connection pool wrapper for SportsQuant TimescaleDB integration.
...
Usage::
    ...
    from sportsquant.infra.db.connection import get_pool, health_check
...
class DBConfig(BaseSettings):
    ...
    Values are sourced from environment variables with the ``SPORTSQUANT_DB_``
    ...
    port: int = 5432
    user: str = "sportsquant"
    password: str = "sportsquant"
    database: str = "sportsquant"

    model_config = {"env_prefix": "SPORTSQUANT_DB_"}  # type: ignore[typed-dict-unknown]

    @classmethod
    def from_env(cls) -> DBConfig:
        """Construct configuration from environment variables.

        Uses ``pydantic-settings`` env prefix ``SPORTSQUANT_DB_`` so that
        ``SPORTSQUANT_DB_HOST`` maps to :attr:`host`, etc.
    ...
    def from_dsn(cls, dsn: str) -> DBConfig:
        ...
        return cls(
            ...
            user=parsed.username or "sportsquant",
            password=parsed.password or "sportsquant",
            database=parsed.path.lstrip("/") or "sportsquant",
        )
