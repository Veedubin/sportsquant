...
from quantitative_sports.util.validation import PRA_CONTEXT_FEATURES, PRA_LABEL_COLUMNS
...
def merge_team_context_data(pra_df: pd.DataFrame, season: int) -> pd.DataFrame:
    ...
    games_path = Path(f"quantitative_sports/data/sources/team_games_{season}.parquet")
    season_path = Path(f"quantitative_sports/data/sources/team_season_{season}.parquet")
