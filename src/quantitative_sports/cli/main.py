"""Quant-Sports CLI — quantitative sports betting toolkit.
...
# when transitive dependencies (e.g. quantitative_sports.util.metrics) are
# not yet available.
...
def _american_to_odds(american: int):
    ...
    Odds = _import("quantitative_sports.core.betting.odds", "Odds")
...
@click.group()
@click.version_option(version="0.1.0", prog_name="quantitative_sports")
def app() -> None:
...
def ev(line: float, odds: int, prob: float) -> None:
    ...
    """
    from quantitative_sports.core.betting.engine import expected_value, kelly_fraction
...
def kelly(
    ...
    """
    from quantitative_sports.core.betting.kelly import KellyCalculator, AdaptiveKellyContext
...
def backtest_run(
    ...
    """
    from quantitative_sports.core.backtest.engine import PraBacktestConfig, backtest_pra_lines
    from quantitative_sports.core.backtest.parallel import backtest_summary
...
def scrape_pinnacle(sport: str, max_games: int) -> None:
    ...
    try:
        from quantitative_sports.data.sources.pinnacle import __dict__ as pinnacle_exports
...
def scrape_injuries(league: str) -> None:
    ...
    try:
        from quantitative_sports.data.sources.espn_injuries import __dict__ as espn_exports
...
def optimize_slip(legs: int, site: str, tier: str, n_sims: int, min_ev: float) -> None:
    ...
    """
    from quantitative_sports.models.analysis.slip_optimizer import SlipOptimizer
    ...
    if site == "prizepicks":
        from quantitative_sports.models.analysis.slip_optimizer import PAYOUT_POWER, PAYOUT_FLEX
...
def metrics(csv: str, pnl_col: str) -> None:
    ...
    """
    from quantitative_sports.core.betting.metrics import (
...
def edge(prob: float, odds: int) -> None:
    ...
    """
    from quantitative_sports.core.betting.kelly import EdgeCalculator
...
def decide(
    ...
    """
    from quantitative_sports.core.betting.engine import decide_over_under, _detect_arbitrage
...
def nfl_ev(
    ...
    from quantitative_sports.models.analysis.rules.nfl import get_nfl_stat_key
...
def nfl_kelly(
    ...
    from quantitative_sports.models.analysis.rules.nfl import get_nfl_stat_key
    from quantitative_sports.core.betting.kelly import KellyCalculator
...
def nfl_backtest(
    ...
    if walk_forward:
        try:
            from quantitative_sports.core.backtest.regime import WalkForwardBacktest, WalkForwardConfig
