"""Schema management utilities for Quant-Sports TimescaleDB integration.

Provides idempotent verification, programmatic creation, migration support,
and teardown of the quantitative_sports database schema.  The canonical DDL lives in
...
Usage::
    ...
    from quantitative_sports.infra.db.schema import verify_schema, create_schema
...
async def create_schema(pool: DatabasePool, if_not_exists: bool = True) -> None:
    """Programmatically create the full quantitative_sports schema.
...
async def drop_schema(pool: DatabasePool, confirm: bool = False) -> None:
    """Drop all quantitative_sports tables, hypertables, and views.
    ...
    logger.warning("Dropping entire quantitative_sports schema!")
