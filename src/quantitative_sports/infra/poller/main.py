"""Poller entrypoint for Quant-Sports v0.2.0.

This module is the console-script entrypoint registered as ``quant-sports-poller``
in ``pyproject.toml``.  It reads :class:`~quantitative_sports.infra.poller.config.PollerConfig`,
dispatches to the appropriate scheduler backend, and provides a Click CLI for
manual invocation.

Scheduler dispatch
-------------------
The ``QUANT_SPORTS_POLLER_SCHEDULER`` environment variable (default: ``"prefect"``)
selects the scheduling backend:

- ``"prefect"`` — calls :func:`~quantitative_sports.infra.poller.flows.serve_flows` which
  starts a long-running Prefect serve loop with interval deployments.  If Prefect
  is not installed, it falls back to
  :func:`~quantitative_sports.infra.poller.flows.run_flows_forever`.

- ``"cron"`` — calls :func:`~quantitative_sports.infra.poller.cron_fallback.run_cron_loop`
  which runs all enabled pollers in a plain ``asyncio`` loop on a fixed interval.

CLI subcommands
---------------
The module can also be invoked directly as ``python -m quantitative_sports.infra.poller.main``
...
from quantitative_sports.infra.poller.cron_fallback import format_cycle_summary, run_cron_loop
from quantitative_sports.infra.poller.tasks import run_health_check, run_poll_cycle
...
async def main_async() -> int:
    ...
    logger.info(
        "Quant-Sports poller starting — scheduler=%s, sports=%s, sources=%s, db_host=%s",
        config.scheduler,
        ",".join(active_sports),
    ...
    try:
        if config.scheduler == "prefect":
            try:
                from quantitative_sports.infra.poller.flows import serve_flows
                ...
                from quantitative_sports.infra.poller.flows import run_flows_forever
...
def main() -> None:
    """Sync wrapper called by the ``quant-sports-poller`` console script.

    If invoked with extra CLI args (e.g. ``quant-sports-poller once`` or
    ``quant-sports-poller status``), dispatches to the Click :func:`poller_cli`
...
@click.group()
def poller_cli() -> None:
    """Quant-Sports poller — data collection scheduler and CLI."""
