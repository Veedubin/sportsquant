"""Odds API poller source for Quant-Sports v0.2.0.
...
prefix (``QUANT_SPORTS_POLLER_ODDS_API_``) so that the poller can be tuned
...
class OddsAPIConfig(BaseSettings):
    ...
    prefix and the ``QUANT_SPORTS_POLLER_ODDS_API_`` prefix are accepted
    ...
    model_config = {  # type: ignore[typed-dict-unknown]
        "env_prefix": "QUANT_SPORTS_POLLER_ODDS_API_",
        "env_aliases": {
