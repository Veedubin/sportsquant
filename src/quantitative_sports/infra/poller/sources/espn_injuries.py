"""ESPN injuries poller source for Quant-Sports v0.2.0.
...
(``QUANT_SPORTS_POLLER_ESPN_``) so that the poller can be tuned independently
...
class ESPNInjuriesConfig(BaseSettings):
    ...
    prefix and the ``QUANT_SPORTS_POLLER_ESPN_`` prefix are accepted
    ...
    user_agent: str = "quantitative_sports/0.2.0"

    model_config = {  # type: ignore[typed-dict-unknown]
        "env_prefix": "QUANT_SPORTS_POLLER_ESPN_",
        "env_aliases": {
