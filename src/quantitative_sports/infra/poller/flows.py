"""Prefect flow definitions for Quant-Sports poller scheduling.

This module wraps the plain-async task functions from
:mod:`~quantitative_sports.infra.poller.tasks` as Prefect flows and tasks so they can
...
from quantitative_sports.infra.poller.tasks import (
    TaskResult,
    run_espn_poll,
    run_health_check,
    run_odds_api_poll,
...
async def _odds_api_poll_task(config: PollerConfig, sport: str) -> TaskResult:
    """Prefect task wrapping :func:`~quantitative_sports.infra.poller.tasks.run_odds_api_poll`.
...
async def _espn_poll_task(config: PollerConfig, sport: str) -> TaskResult:
    """Prefect task wrapping :func:`~quantitative_sports.infra.poller.tasks.run_espn_poll`.
...
async def _health_check_task(config: PollerConfig) -> TaskResult:
    """Prefect task wrapping :func:`~quantitative_sports.infra.poller.tasks.run_health_check`.
...
def serve_flows(config: PollerConfig) -> None:
    ...
    if not PREFECT_AVAILABLE:
        raise RuntimeError(
            ...
            "Install the [poller] extra:  pip install quantitative_sports[poller]"
        )
