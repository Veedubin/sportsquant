"""Poller configuration model for Quant-Sports v0.2.0.
...
Environment variables use the ``QUANT_SPORTS_POLLER_`` prefix.  Nested configs
retain their own prefixes (e.g. ``QUANT_SPORTS_POLLER_ODDS_API_``) and are
...
from quantitative_sports.infra.poller.sources.espn_injuries import ESPNInjuriesConfig
...
class PollerConfig(BaseSettings):
    ...
    ``QUANT_SPORTS_POLLER_`` prefix.  Nested sub-configs retain their own
    prefixes (e.g. ``QUANT_SPORTS_POLLER_ODDS_API_``) and are resolved
    ...
    model_config = SettingsConfigDict(  # type: ignore[typed-dict-unknown]
        env_prefix="QUANT_SPORTS_POLLER_",
    )
