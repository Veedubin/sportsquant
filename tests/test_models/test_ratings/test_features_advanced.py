...
from quantitative_sports.features_advanced.raptor_box import (
    RaptorBoxConfig,
    RaptorBoxFeatures,
    _safe_divide,
    _rolling_mean_lagged,
)
from quantitative_sports.features_advanced.raptor_composite import RaptorCompositeFeatures
from quantitative_sports.features_advanced.massey_ratings import MasseyRatings
...
class TestSafeDivide:
    ...
    def test_divide_by_zero(self):
        """Test divide by zero returns fill value."""
        numerator = pd.Series([10, 20, 30])
    ...
    def test_all_zeros(self):
        """Test all zeros returns fill value."""
...
class TestRollingMeanLagged:
    ...
    def test_basic_rolling_mean(self):
        ...
        # Series: [1, 2, 3, 4, 5]
        # After shift(1): [NaN, 1, 2, 3, 4]
        # Rolling 3 on shifted: [NaN], [NaN, 1], [NaN, 1, 2], [1, 2, 3], [2, 3, 4]
        # With min_periods=1: [NaN], [1.0], [1.5], [2.0], [3.0]
        assert pd.isna(result.iloc[0])  # First is NaN (no data in window)
        np.testing.assert_allclose(float(result.iloc[1]), 1.0)  # mean of [1] after shift
        np.testing.assert_allclose(float(result.iloc[2]), 1.5)  # mean of [1, 2] after shift
        np.testing.assert_allclose(float(result.iloc[3]), 2.0)  # mean of [1, 2, 3] after shift
        np.testing.assert_allclose(float(result.iloc[4]), 3.0)  # mean of [2, 3, 4] after shift

    def test_short_series(self):
        """Test rolling mean with short series."""
        series = pd.Series([10, 20])
        result = _rolling_mean_lagged(series, 5)
        # Window larger than series, should have min_periods=1
...
class TestRaptorBoxFeatures:
    ...
    def test_scoring_features(self):
        """Test scoring efficiency features computation."""
    ...
    def test_rebounding_features(self):
        """Test rebounding features computation."""
        config = RaptorBoxConfig()
        features = RaptorBoxFeatures(config)
    ...
    def test_defensive_features(self):
        """Test defensive features computation."""
        config = RaptorBoxConfig()
        features = RaptorBoxFeatures(config)
    ...
    def test_compute_all_box_features(self):
        """Test computing all box score features."""
        config = RaptorBoxConfig()
        features = RaptorBoxFeatures(config)
...
class TestRaptorCompositeFeatures:
    ...
    def test_compute_raptor_percentile(self):
        """Test RAPTOR percentile computation."""
        composite = RaptorCompositeFeatures()
    ...
    def test_normalize_raptor_for_market(self):
        """Test RAPTOR normalization for specific markets."""
        composite = RaptorCompositeFeatures()
...
class TestMasseyRatings:
    ...
    def test_compute_ratings_empty_games(self):
        """Test that empty games raises error."""
        ratings = MasseyRatings(home_advantage=3.0)
        ...
        games = pd.DataFrame(
            {
                "HOME_TEAM": ["LAL", "BOS"],
                "AWAY_TEAM": ["BOS", "LAL"],
                # Missing HOME_SCORE and AWAY_TEAM_score
            }
        )
    ...
    def test_decompose_rating(self):
        """Test rating decomposition."""
        ratings = MasseyRatings(home_advantage=3.0)

        off, deff = ratings.decompose_rating(2.0)
        assert off > deff  # Positive rating should favor offense

        off, deff = ratings.decompose_rating(-2.0)
        # Negative rating: split = max(-2, -2) / -2 = 1.0
        # offensive = base_offense + (1 - 1) * (-2) = 0
        # defensive = base_defense + 1 * (-2) = -2
        # So defensive is lower (more negative), meaning worse defense
        # The logic is that negative overall means team is below average
        # For betting purposes, negative is bad for the team
    ...
    def test_rest_day_adjustment(self):
        ...
        # 2 rest days with 0.15 coefficient
    ...
    def test_conference_adjustment(self):
        ...
        # West mean should be higher than East mean
